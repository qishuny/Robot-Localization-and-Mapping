...
def init_landmarks(init_measure, init_measure_cov, init_pose, init_pose_cov):
    ...
    # beta i is inital_measure i*2
    # r i is inital_measure i*2+1
    for i in range(k):
        betaTemp = init_measure[i*2][0]
        rTemp = init_measure[i*2+1][0]
        xTemp = x + rTemp * np.cos(theta+betaTemp)
        yTemp = y + rTemp * np.sin(theta+betaTemp)

        Ht = np.array([[-rTemp*np.sin(betaTemp+theta), np.cos(betaTemp+theta)],
                      [rTemp*np.cos(betaTemp+theta), np.sin(betaTemp+theta)]])

        landmark[i*2][0] = xTemp
        landmark[i*2+1][0] = yTemp
...
def update(X_pre, P_pre, measure, measure_cov, k):
    ...
    for i in range(k):
        dx = X_pre[3+i*2][0]-X_pre[0][0]
        dy = X_pre[3+i*2+1][0]-X_pre[1][0]
        x2y2 = dx**2+dy**2
        sx2y2 = np.sqrt(x2y2)
        Ht[i*2][0] = dy/x2y2
        Ht[i*2][1] = dx/x2y2
        ...
        # Ht[i*2][3+i*2] = 1
        # Ht[i*2+1][4+i*2] = 1
        Ht[i*2][3+i*2] = -dy/x2y2
        Ht[i*2][4+i*2] = -dx/x2y2
        Ht[i*2+1][3+i*2] = dx/sx2y2
        Ht[i*2+1][4+i*2] = dy/sx2y2
        Qt[i*2:i*2+2, i*2:i*2+2] = measure_cov
        Hu[i*2][0] = warp2pi(np.arctan2(dy, dx)-X_pre[2][0])
        Hu[i*2+1][0] = sx2y2

    Kt = P_pre @ Ht.T @ np.linalg.inv((Ht @ P_pre @ Ht.T + Qt))

    updateX = Kt@(measure-Hu)

    updateP = (np.eye(3 + 2 * k)-Kt @ Ht)
...
def evaluate(X, P, k):
    ...
    for i in range(k):
        ...
        Edif[i] = np.sqrt(dx**2+dy**2)

        dif = np.array([dx, dy])

        cov = P[3+i*2:3+i*2+2, 3+i*2:3+i*2+2]

        Mdif[i] = np.sqrt(dif@cov@dif.T)

        print("the Euclidean distance of landmark", i+1, "is", Edif[i])
        print("the Mahalanobis distance of landmark", i+1, "is", Mdif[i])
