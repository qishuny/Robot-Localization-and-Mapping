...
def find_projective_correspondence(source_points,
    ...
    qi = target_vertex_map[target_vs,target_us,:]
    diff = qi-T_source_points 
...
def build_linear_system(source_points, target_points, target_normals, T):
    ...
    for i in range(M):
        ...
        A_temp = np.array([[0,-pi_prime[2,0],pi_prime[1,0]],
        [pi_prime[2,0],0,-pi_prime[0,0]],
        [-pi_prime[1,0],pi_prime[0,0],0]])
        nqi =n_q[i,:]
        A[i,:] = nqi.T @ np.hstack((-A_temp,np.eye(3)))
        b[i,] = -nqi.reshape(1,3) @(pi_prime-q[i,:].reshape(3,1))
    return A, b
