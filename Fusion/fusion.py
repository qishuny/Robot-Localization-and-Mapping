...
class Map:
    ...
    def merge(self, indices, points, normals, colors, R, t):
        ...
        vgu = (R @ points.T +t).T
        ck = self.weights[indices,:]
        
        self.points[indices,:] = (ck*self.points[indices,:]+vgu)/(ck+1)

        ngu = (R @normals.T).T
        norms = (self.weights[indices,:]*self.normals[indices,:]+ngu)/(self.weights[indices,:]+1)
        self.normals[indices,:] = norms/np.tile(np.linalg.norm(norms,axis=1),(3,1)).T
    ...
    def add(self, points, normals, colors, R, t):
        ...
        N = points.shape[0]
        vgu = (R @ points.T +t).T
        self.points = np.concatenate((self.points,vgu))

        ngu = (R @normals.T).T
        self.normals = np.concatenate((self.normals,ngu))
    ...
    def filter_pass2(self, points, normals, input_points, input_normals,
        ...
        norms = (input_normals*normals).sum(axis=1)
        norms_mag = np.sum(np.abs(input_normals)**2,axis=-1)**(1./2)*np.sum(np.abs(normals)**2,axis=-1)**(1./2)
        
        angle = np.arccos(norms/(norms_mag))

        diff = points-input_points 
        distance = np.sqrt((diff**2).sum(axis=1))
        
        mask = np.zeros(points.shape[0]).astype(bool)
        mask[(distance<=dist_diff)&(np.abs(angle)<angle_diff)]= True
        return mask
...
if __name__ == '__main__':
    ...
    totalPoints = m.points.shape[0]
    totalInput = 200*source_vertex_map.shape[0]*source_vertex_map.shape[1]
    print("number of points", totalPoints)
    print("compression ratio is", totalPoints/totalInput*100)
